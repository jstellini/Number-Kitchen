...
- **A recipe** – four to seven stages in sequence. Each shows a numeral and asks for that many of
  ...
  - **Count and place** – put that many scoops, eggs, pepperoni or cherries in.
  - **Count the action** – roll, stir, whisk, spread or flip that many times.
  - **Set the dial** – turn the oven or blender up until it reads the number shown.
  - **Cut** – swipe until there are that many pieces. It counts *pieces*, not cuts.
  - **Serve** – give one to each customer, which is one-to-one correspondence with a reason to care.
...
- **The café** – everything she has cooked sits on the counter and reacts to a poke.
...
- **Ingredient and dish art** – `ART` and `DISH_ART` in `js/data.js`, flat SVG in a 100×100 and
  200×140 box. Where items may land inside a vessel is the matching `.v-*` rule in `css/style.css`,
  so the geometry lives with the art.
...
- **Voice** – every line is a pre-generated mp3 (Microsoft Edge neural TTS, `en-GB-SoniaNeural`)
  ...
  sentence — about 490 clips in total.
  ```
...
js/data.js            digit strokes, ingredient art, the recipe book, the cast
